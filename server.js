...
* Send a POST request to localhost:3000/api with body
* {
*   "name": "Bob"
* }
*/
app.post("/api", function (request, response) {
	...
	var N = request.body.N_value;
	var p = request.body.p_value;
	var q = request.body.q_value;
	var lst = [];

//generate sequence
for(var i=1;i<=N;i++){
    if(i%p == 0 && i%q ==0 && i!=p && i!=q){
...
var final = lst.join();
var res = final.replace(/,/g," ");
console.log("Sequence is :"+res);
	response.send(res);
});
